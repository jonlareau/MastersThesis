...
#include <sp/sphere.h>
char usage[] = "Usage:  %s [-vf] -o  [ short_10 | short_01 | short_natural | ulaw ] { filein | - } { fileout | - }\n        %s [-vi] -o [ short_10 | short_01 | short_natural | ulaw ] file1 file2 ...\n";
char * prog;
...
char *argv[];
{
	...
	switch ( c ) {
	  ...
	  case 'o':
		...
		fprintf(stderr,"Sorry, ULAW conversion has not yet been implmented\n");
		exit(-1);
	    }
	    else{
...
char *filein, *fileout, *format_conversion;
{
    ...
    { char *buff;
	  ...
	  if (samples_read > 0) {
	      ...
	      if (sp_eof(sp_in) == 0) {
		  printf("Error: Zero samples read while not at EOF\n");
		  sp_close(sp_in); sp_close(sp_out);
		  if (! strsame(fileout,"-")) unlink(fileout);
		  mtrf_free(buff);
		  return(100);
	      }
	      if ((!sp_eof(sp_in)) || (sp_error(sp_in) >= 100)) { /* a checksum error occured, close the sp and */
		  sp_print_return_status(stdout);
		  sp_close(sp_in); sp_close(sp_out);
		  if (! strsame(fileout,"-")) unlink(fileout);
		  mtrf_free(buff);
