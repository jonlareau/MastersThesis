...
/*  of size 'x' by 'y'                                                */
/**********************************************************************/
init_disk_array(filename,x,y,dbg)
char *filename;
int x,y;
int dbg;
{
...
DISK_T value;
{
   ...
   check_arg(x,y);
   new_loc = (x + (array_y * y)) * array_elem_size;
...
/*   printf("      moved to %d\n",array_cur_pos);*/
   ...
   if (fwrite(&value,1,array_elem_size,array_fp) != array_elem_size){
...
/*   printf("      moved to %d\n",array_cur_pos);*/
   ...
   if (fread(value,1,array_elem_size,array_fp) != array_elem_size){
...
/**********************************************************************/
disk_compute_entropy(dbg, entropy_out)
int dbg;
float *entropy_out;
{
