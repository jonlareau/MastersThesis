...
/*  file                                                     */
cfg_get_args(cfg,argc,argv,setup_config)
CONFIG *cfg;
int argc;
char **argv, *setup_config;
{
    ...
    { char *cfg_file;
      ...
      if (*cfg_file != NULL_CHAR){
          load_CONFIG(cfg,cfg_file,/*silent=*/FALSE);
...
/*   structure.                                                 */
/****************************************************************/
load_com_line_CONFIG(cfg,argc,argv,silent)
CONFIG *cfg;
int argc;
char **argv;
int silent;
{
    ...
    for (arg=1; arg < argc;){
        if ((cfg_ind = is_CONFIG_id(cfg,argv[arg]+1)) != NOT_CONFIG_ID){
            switch (cfg->rec_list[cfg_ind].value_type){
                case CFG_TGL:
                    cfg->rec_list[cfg_ind].value = (char *)TOGGLE_ON;
                    arg++;
                    break;
                case CFG_C:
                case CFG_STR:
    ...
    while (*(cfg->rec_list[i].intern_id) != NULL_CHAR){
        if ((i != cfg_ind) && (cfg->rec_list[cfg_ind].flag_group ==
            ...
            switch (cfg->rec_list[i].value_type){
                   case CFG_TGL:
                       cfg->rec_list[i].value = (char *)TOGGLE_OFF;
                       break;
                   case CFG_C:
                   case CFG_STR:
...
char *str;
{
    ...
    if ((cfg_ind = is_CONFIG_id(cfg,str)) != NOT_CONFIG_ID)
        return ((int)cfg->rec_list[cfg_ind].value);
    fprintf(stderr,"Error: Unknown Config Paramater Requested %s\n",str);
    exit(-1);
}
...
/****************************************************************/
set_CONFIG_elem_TGL(cfg,str,value)
CONFIG *cfg;
char *str;
int value;
{
    ...
    cfg->rec_list[cfg_ind].value = (char *)value;
}

/****************************************************************/
/*  Set the value for the CFG_C value based on the id string    */
/****************************************************************/
set_CONFIG_elem_C(cfg,str,value)
CONFIG *cfg;
char *str;
char *value;
{
...
/*  Return the element count for the id string                  */
/****************************************************************/
CONFIG_elem_present_C2(cfg,str)
CONFIG *cfg;
char *str;
{
    int ind, i;
