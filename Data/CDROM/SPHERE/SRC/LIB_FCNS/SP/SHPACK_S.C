...
int shortpack_compress(fpin, fpout, status) FOB *fpin, *fpout; char *status; {
  int exitcode;
...
int shortpack_uncompress(fpin, fpout, header) 
FOB *fpin, *fpout;
...
/*    printf(" Read length = %d\n",waveform_len);    */
    ...
    write_len = fob_fwrite(waveform,2,waveform_len,fpout);
/*    printf(" Write length = %d\n",write_len);*/
	...
	if (num_in_chunk < 0)
	    return(-1);
	written=fob_fwrite(array,2,num_in_chunk,fpout);
	printf("  %d written\n",num_in_chunk);
...
read_shortpack_header(ifp, arg, pfilesize, pswap)
FOB *ifp;
int arg;
int *pfilesize;
int *pswap;
{
  ...
  *pfilesize = ntohl(getw(ifp));
  flags = ntohl(getw(ifp));
