...
read_wav_data(fp, pwaveform, header, check_checksum)
FOB *fp;
short **pwaveform;
header_t *header;
int check_checksum;
{ int i;
  ...
  sp_get_data(header,"sample_count",&len,&string_len);
...
/*  if((strcmp(sample_byte_format, "01") == 0) || (strcmp(sample_byte_format, "10") == 0))*/
  ...
  { fread(waveform, sizeof(short), len, fp);
    ...
    if ((htons(test_short) == test_short && strcmp("01", sample_byte_format) == 0) ||
	(htons(test_short) != test_short && strcmp("10", sample_byte_format) == 0))
    { printf("swapping");
...
/*      printf("reading shortpack");*/
    ...
    { printf("SORRY! don't know about sample_byte_format %s\n", sample_byte_format);
      exit(-1);
    }
  }
  if(check_checksum)
  { string_len = sizeof(int);
    if(sp_get_data(header,"sample_checksum",&checksum,&string_len) < 0)
...
write_wav_data(fp, waveform, header)
FOB *fp;
short *waveform;
header_t *header;
{ int i;
...
update_checksum_unless_its_already_there(waveform, header)
short *waveform;
header_t *header;
{ int field_len;
  int checksum;
  ...
  if(sp_get_data(header,"sample_checksum",&checksum,&field_len) < 0)
  { field_len = sizeof(int);
    if(sp_get_data(header,"sample_count",&wav_len,&field_len) < 0)
    { printf("HEY! couldn't read sample count from header!!\n");
      exit(-1);
    }
    checksum = compute_checksum(waveform, wav_len);
    sp_add_field(header,"sample_checksum",T_INTEGER,&checksum);
  }
}

update_checksum_even_if_its_already_there(waveform, header)
short *waveform;
header_t *header;
{ int field_len;
  int checksum;
  ...
  field_len = sizeof(int);
  if(sp_get_data(header,"sample_checksum",&checksum,&field_len) < 0)
  { sp_change_field(header,"sample_checksum",T_INTEGER,&checksum);
  }
  else
  { sp_add_field(header,"sample_checksum",T_INTEGER,&checksum);
  }
