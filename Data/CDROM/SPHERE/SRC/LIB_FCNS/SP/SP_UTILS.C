#ifndef lint
  static char rcsid[] = "$Header: /home/beldar/stan/sphere/RCS/sp_utils.c,v 1.4 1993/03/25 00:20:51 stan Exp stan $";
#endif
...
/*    the new header.                                                     */
/**************************************************************************/
FUNCTION struct header_t *sp_dup_header(h)
struct header_t *h;
{
    ...
    if ( duph == HDRNULL ) {
	fprintf(stderr,"Error: Unable to dup header, can't allocate mem.\n");
	return(HDRNULL);
    }
...
/**********************************************************************/
long sp_header_size(h)
struct header_t *h;
{
    FILE *fp;
    long header_size, data_size;    
    ...
    if (fp == FPNULL){
	if (sp_verbose > 15) printf("Can't open file '%s'\n",rsprintf("%s/%s.sz",TEMP_DIR,TEMP_BASE_NAME));
	return -1;
    }
