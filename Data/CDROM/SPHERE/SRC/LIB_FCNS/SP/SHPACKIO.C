...
write_shortpacked_data(array, num_elements, fp)
short *array;
int num_elements;
FOB *fp;
{ int i,j,k;
...
write_shortpack_reading_from_file(ifp, ofp, swap)
FOB *ifp;
FOB *ofp;
int swap;
{ int i,j,k;
...
read_shortpack_writing_to_file(ifp, ofp, swap)
FOB *ifp;
FOB *ofp;
int swap;
{ int i,j,k;
  ...
  { if(next_write == next_read)
	...
	{ temp_char = *char_ptr;
	  ...
	  *(char_ptr++) = temp_char;
      ...
      { printf("read_shortpack_writing_to_file: HAD_TROUBLE_WRITING!!\n");
	exit (-1);
      }
      else
      { next_read = 0;
 ...
 */
find_next_chunk_to_compress(buffer, start, num_elements, pbits, mask)
short *buffer;
int start;
int num_elements;
int *pbits;
...
better_to_chop_here_vs_using_more_bits(bits_for_element, bits_for_chunk, 
				       num_in_chunk)
int bits_for_element, bits_for_chunk, num_in_chunk;
{ int els_so_far;
  int new_els;
...
better_to_chop_here_and_use_fewer_bits_for_future(bits_for_element,
						  bits_for_chunk, buffer, 
						  bits_for_element_cache, 
						  cache_start, start, end, 
						  mask)
short *buffer;
int bits_for_element, bits_for_chunk;
short *bits_for_element_cache;
int start, end, mask;
 ...
 */
int
pack_and_write_chunk_to_file(buffer, start, num, bits, fp, mask)
FOB *fp;
short *buffer;
int start;
int num, bits;
int mask; /*so the buffer can be a circular buffer 
 ...
 */
int
read_shortpack_chunk_from_file(fp, buffer, start, max_elements, mask)
FOB *fp;
short *buffer;
int start, max_elements;
int mask; /*so the buffer can be a circular buffer (if ALL_ONES, 
...
/*  printf("num_read %d != els %d\n", num_read,els);
  ...
  if(num_read != els)
  { printf("read_shortpack_chunk_from_file: HEY! something is seriously wrong!  maybe you\n");
...
pack_short_array_into_buffer(buffer, start, num, bits, mask, compressed_data)
short *buffer;
int num, bits, mask;
...
unpack_short_array_into_buffer(buffer, start, max_elements, bits, mask, 
			       compressed_data, num)
short *buffer;
int start, max_elements;
int mask;
